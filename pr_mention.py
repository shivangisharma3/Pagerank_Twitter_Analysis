...
def pagerank(tx,top_count):
	for record in tx.run("CALL algo.pageRank.stream('MATCH(u:USER) return id(u) as id','MATCH (u1:USER)-[:TWEETED]-(u2:TWEET)-[:HAS_MENTION]->(u7:USER)  RETURN 		id(u1) as source, id(u7) as target',{graph:'cypher'}) "
	"YIELD node ,score with node,score order by score desc limit $top_count " 
	"RETURN node{.name}, score",top_count=top_count):
		print(record["node"]['name']," ",record["score"])
	



def pagerank44(tx,name,top_count,near_count):
	n_id=[0]
	n_dist=[65565]
	tx.run("MATCH (u1:USER)-[:TWEETED]-(u2:TWEET)-[:HAS_MENTION]-(u7:USER) "
	"WITH u1,u7 "
	"MERGE (u1)-[:point]->(u7) ")
	tx.run("MATCH (a:USER),(b:USER) WHERE a.name = 'James Markey' AND b.name = 'Jupol Jaén' "
		"WITH a,b "
		"MERGE (b)-[:point]->(a) ")
	tx.run("MATCH (a:USER),(b:USER) WHERE a.name = 'James Markey' AND b.name = 'Ville de Beauvais' "
		"WITH a,b "
		"MERGE (b)-[:point]->(a) ")
	...
	"RETURN node{.name}, score",top_count=top_count):
		...
		"{nodeQuery:'USER', relationshipQuery:'point',defaultValue:1.0, direction:'BOTH'} ) "
		"YIELD nodeId,cost "
...
with driver.session() as session:
	...
	session.write_transaction(pagerank44,name,num,num2)
	#session.write_transaction(pagerank,num)
